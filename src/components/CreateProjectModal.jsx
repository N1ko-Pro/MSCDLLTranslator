...
import { FolderPlus, Settings, Check, X } from 'lucide-react';

export default function CreateProjectModal({ onClose, modData, onCreate }) {
  const [projectName, setProjectName] = useState(() => {
    return modData?.name && modData?.name !== 'Unknown' ? `${modData.name}_RUS` : 'Мой_Перевод_RUS';
  });
  
  const [author, setAuthor] = useState('Неизвестен');
      ...
      <div className="bg-[#131316] border border-white/10 rounded-2xl p-6 w-full max-w-md shadow-[0_30px_100px_rgba(0,0,0,0.5)] relative overflow-hidden">
        ...
        <div className="flex items-center justify-between mb-8 pb-4 border-b border-white/5">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-lg flex items-center justify-center border bg-emerald-500/10 border-emerald-500/20">
            ...
            <div className="flex flex-col">
              ...
              <p className="text-[12px] font-medium text-zinc-500">
                Заполните начальные данные для проекта
              </p>
            </div>
          </div>
          <button
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-5">
          <div className="flex flex-col gap-2">
            ...
            <input
              ...
              className="w-full bg-[#0f0f13] border border-white/10 rounded-xl px-4 py-3 text-sm text-zinc-200 placeholder:text-zinc-600 focus:outline-none focus:border-emerald-500/50 focus:ring-2 focus:ring-emerald-500/20 transition-all font-medium"
              autoFocus
            />
          </div>
          ...
          <div className="flex items-center gap-4 mt-4">
            ...
            <button
              ...
              className="flex-[2] flex justify-center items-center gap-2 py-3 px-4 rounded-xl text-sm font-bold text-white transition-all shadow-lg active:scale-95 disabled:opacity-50 disabled:pointer-events-none bg-emerald-600 hover:bg-emerald-500 shadow-emerald-600/20 border-emerald-500 border"
            >
