  ...
  return (
    <div className="fixed bottom-6 right-6 z-[9999] flex flex-col gap-3 items-end pointer-events-none">
      {notifications.map(n => {
        ...
        }[n.type] || { icon: Info, color: 'text-blue-400', bg: 'bg-blue-500/10', border: 'border-blue-500/20' };
        ...
        return (
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              {n.title && <h4 className="text-[13px] font-bold text-white tracking-wide">{n.title}</h4>}
              {n.message && <p className="text-xs text-zinc-400 mt-1.5 leading-relaxed font-medium">{n.message}</p>}
            </div>
            <button 
