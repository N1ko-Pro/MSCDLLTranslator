...
export default function TranslationStatusBar({ visible, stage, progress }) {
  if (!visible) return null;

  const safeProgress = Math.max(0, Math.min(100, progress ?? 0));
  const stageLabel = stage || 'Перевод строк...';
  ...
  return (
    <div className="absolute top-0 left-0 right-0 z-50 bg-white/[0.03] backdrop-blur-2xl border-b border-white/10 h-20 flex items-center justify-between px-8 shadow-2xl animate-[slideInFromTop_0.5s_cubic-bezier(0.34,1.56,0.64,1)] overflow-hidden">
      ...
      <div className="absolute inset-0 bg-gradient-to-r from-fuchsia-500/5 via-transparent to-fuchsia-500/5 opacity-50 pointer-events-none" />
      ...
      <div className="relative flex items-center justify-between w-full z-10">
        <div className="flex items-center gap-5">
          <div className="relative flex items-center justify-center w-[46px] h-[46px]">
            ...
            <Languages className="w-6 h-6 text-fuchsia-300 drop-shadow-[0_0_8px_rgba(192,38,211,0.6)]" />
          </div>
          <div>
            <div className="flex items-center gap-3">
              <h4 className="text-sm font-extrabold text-white tracking-wide drop-shadow-md">АВТО-ПЕРЕВОД...</h4>
            </div>
        ...
        <div className="flex-1 max-w-2xl mx-10">
          <div className="w-full h-3 bg-black/30 backdrop-blur-md rounded-full overflow-hidden border border-white/10 relative shadow-[inset_0_2px_5px_rgba(0,0,0,0.4)]">
            ...
            />
            <div className="absolute inset-0 bg-gradient-to-b from-white/20 to-transparent w-full h-full rounded-full mix-blend-overlay" />
          </div>
